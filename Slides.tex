...
Let $det_M$ be the determinant of the $n \times n$ matrix $M \in \operatorname{GL}_n k$ with entries from a field $k$ and $k^*$ is the multiplicative group of $k$. Both ${\bf GL}_n$ and $*$ are functors from the category of fields to the category of groups, and $det_M : {\bf GL}_n k \to k^*$ is a morphism of groups and it is natural:
\centerline{
\xymatrix{
...
\begin{frame}
\frametitle{Kripke-Joyal semantics for predicate intuitionistic modal logics}
\end{frame}
