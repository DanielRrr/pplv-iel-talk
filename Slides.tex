...
This talk is based on the author's master thesis defended at Lomonosov Moscow State University (the faculty of philosophy, the department of logic) under supervision by Vladimir Vasyukov and Vladimir Krupski in 2018. See the extended version of the thesis published in Journal of Logic and Computation:

\begin{itemize}
\item Daniel Rogozin. Categorical and algebraic aspects of the intuitionistic modal logic ${\bf IEL}^-$ and its predicate extensions, 2021.
\end{itemize}
\end{frame}
...
\item Kripke-Joyal semantics and modal Heyting algebra representations
\end{enumerate}
...
\item Side effects are isolated within a monad and the relevant ``effected'' code is purely functional.
\end{itemize}
\end{small}
}
\end{frame}
...
\item Applicative is a weaker version of monadic computation where elements of a pipeline do not depend on one another.
...
\item An example of local truth in topology is local equality or being constant (for continuous functions).
